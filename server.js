...
function avatar(name) {
  ...
  return `<div class="avatar">${escapeHtml(first)}</div>`;
}
...
.purple{background:#6a4c93}
.gray{background:#687078}
...
return sendHtml(res,200,'ثبت‌نام',`
<div class="card">
...
<input name="password" type="password" minlength="6" placeholder="رمز عبور، حداقل ۶ کاراکتر" required>

<button class="green full">ثبت‌نام</button>
...
return sendHtml(res,400,'خطا',
`<div class="card error">اطلاعات واردشده معتبر نیست.</div>`,null);
}
...
return redirect(res,'/',setSessionCookie(sessionId));
}
...
<a class="btn" href="/new-post">➕ انتشار پست</a>

<a class="btn gray" href="/jobs">💼 کاریابی</a>
...
${post.media_url?`
<a href="${escapeHtml(post.media_url)}" target="_blank" rel="noopener">
<img class="media" src="${escapeHtml(post.media_url)}" alt="رسانه پست">
</a>
`:''}
...
<a class="btn gray" href="/post?id=${post.id}">
💬 نظر
</a>

${post.user_id===user.id?`

<a class="btn" href="/edit-post?id=${post.id}">
✏️ ویرایش
</a>
...
<a class="btn gray" href="/report?post=${post.id}">
🚩 گزارش
</a>
...
return sendHtml(res,200,'خانه',content,user);
}

if(method==='GET'&&path==='/new-post'){

return sendHtml(res,200,'پست جدید',`
<div class="card">

<form method="post" action="/new-post">

<textarea name="content" maxlength="5000" placeholder="چه خبر؟" required></textarea>

<input name="media_url" type="url" placeholder="لینک عکس یا ویدیو، اختیاری">

<button class="green full">انتشار پست</button>
...
VALUES($1,$2,'like',$3)`,
[post.rows[0].user_id,user.id,id]
);
}
...
if(!post.rowCount){
return sendHtml(res,404,'پست',
`<div class="card">پست پیدا نشد.</div>`,user);
}
...
${p.media_url?`
<img class="media" src="${escapeHtml(p.media_url)}">
`:''}
...
<textarea name="content" maxlength="2000" placeholder="نظر بنویس..." required></textarea>
...
return sendHtml(res,200,'پست',content,user);
}
...
return sendHtml(res,200,'ویرایش پست',`
<div class="card">
...
<input name="media_url" type="url" value="${escapeHtml(p.media_url||'')}">
...
<input name="q" value="${escapeHtml(q)}" placeholder="جستجوی نام یا ایمیل">
...
return sendHtml(res,200,'جستجو',content,user);
}
...
if(!person.rowCount){
return sendHtml(res,404,'کاربر',
`<div class="card">کاربر پیدا نشد.</div>`,user);
}
...
${button(
`/follow?id=${id}`,
following.rowCount?'➖ لغو دنبال':'➕ دنبال کردن',
'blue'
)}
...
<a class="btn red" href="/report?user=${id}">
🚩 گزارش
</a>
...
${post.media_url?`
<img class="media" src="${escapeHtml(post.media_url)}">
`:''}
...
return sendHtml(res,200,'پروفایل',content,user);
}
...
return sendHtml(res,200,'پروفایل من',`
<div class="card">
...
<span>پست: ${s.posts}</span>

<span>دنبال‌کننده: ${s.followers}</span>

<span>دنبال‌شده: ${s.following}</span>
...
return sendHtml(res,200,'ویرایش پروفایل',`
<div class="card">
...
return sendHtml(res,200,'تغییر رمز',`
<div class="card">
...
<input name="new" type="password" minlength="6" placeholder="رمز جدید" required>
...
[user.id,hashPassword(oldPassword)]
);
...
<a class="btn" href="/post?id=${post.id}">
مشاهده
</a>
...
return sendHtml(res,200,'ذخیره‌شده‌ها',content,user);
}

if(method==='GET'&&path==='/notifications'){

const result=await pool.query(`
SELECT n.*,a.name AS actor
...
if(notification.type==='like') text='پست شما را پسندید.';
if(notification.type==='comment') text='روی پست شما نظر داد.';
if(notification.type==='follow') text='شما را دنبال کرد.';
if(notification.type==='message') text='برای شما پیام فرستاد.';
...
return sendHtml(res,200,'اعلان‌ها',content,user);
}
...
return sendHtml(res,200,'پیام‌ها',`
<div class="card">
...
<textarea name="content" maxlength="3000" placeholder="پیام..." required></textarea>
...
return sendHtml(res,200,'پیام',content,user);
}
...
if(content&&otherId&&otherId!==user.id){
...
<input type="hidden" name="user" value="${reportedUser}">

<textarea name="reason" maxlength="1000" placeholder="دلیل گزارش" required></textarea>
...
const postId=Number(b.get('post')||0)||null;
const reportedUser=Number(b.get('user')||0)||null;
const reason=(b.get('reason')||'').trim();
...
VALUES($1,$2,$3,$4)`,
[user.id,postId,reportedUser,reason]
);
}
...
<a class="btn green" href="/new-job">
➕ ثبت آگهی شغلی
</a>
...
return sendHtml(res,200,'کاریابی',content,user);
}

if(method==='GET'&&path==='/new-job'){

return sendHtml(res,200,'آگهی شغلی جدید',`
<div class="card">
...
<input name="salary" placeholder="حقوق">

<textarea name="description" placeholder="توضیحات آگهی" required></textarea>
...
const title=(b.get('title')||'').trim();
const city=(b.get('city')||'').trim();
const salary=(b.get('salary')||'').trim();
const description=(b.get('description')||'').trim();
...
return sendHtml(res,404,'صفحه پیدا نشد',`
<div class="card">
